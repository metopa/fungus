 ...
 * types, and Object values. This means that all SL types that are objects are
 * handled by the {@link #readObject} method. <p> We use the primitive type only
 * when the same primitive type is uses for all writes. If the local variable is
 * type-polymorphic, then the value is always stored as an Object, i.e.,
 * primitive values are boxed. Even a mixture of {@code long} and {@code
 * boolean} writes leads to both being stored boxed.
 */
...
public abstract class SLReadLocalVariableNode extends SLExpressionNode {
    ...
    protected long readLong(VirtualFrame frame) {
        /*
         * When the FrameSlotKind is Long, we know that only primitive long values
         * have ever been written to the local variable. So we do not need to check
         * that the frame really contains a primitive long value.
         */
